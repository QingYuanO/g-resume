...
export default function EducationForm() {
  ...
  const form = useSyncFields(educationSchema, { education });
  const { fields, append, remove } = useFieldArray({
    control: form.control, // control props comes from useForm (optional: if you are using FormContext)
    name: "education", // unique name for your Field Array
  });
  const educationOpenKey = useOpenKeyStore((state) => state.educationOpenKey);
      ...
      <div className="flex flex-col gap-y-4">
        ...
        >
          {fields.map((item, index) => {
            ...
            const watchRangeDate = form.watch(`education.${index}.rangeDate`);
