...
export default function WorkExperienceForm() {
  ...
  const form = useSyncFields(workExperienceSchema, { workExperience });
      ...
      <div className="flex flex-col gap-y-4">
        ...
        >
          {fields.map((item, index) => {
            ...
            const watchPosition = form.watch(
...
const Projects = ({ index }: { index: number }) => {
  const form = useFormContext<WorkExperienceSchemaType>();
  const { fields, append, remove } = useFieldArray({
  ...
  const getProjectFieldName = (idx: number, name: string) => {
    return `workExperience.${index}.projects.${idx}.${name}` as FieldPath<WorkExperienceSchemaType>;
  };
  return (
